...
def create_maze_graph(topology, size):
    ...
    for room_x, room_y in [
        ...
        for i in range(0, room_size):
            for j in range(1, room_size):
                graph.add_edge(
                    (room_x + i, room_y + j, 3), (room_x + i, room_y + j - 1, 3)
                )
    ...
    for room_x, room_y in [
        ...
        for i in range(0, room_size - 1):
            for j in range(0, room_size):
                graph.add_edge(
                    (room_x + i, room_y + j, 0), (room_x + i + 1, room_y + j, 0)
                )
    ...
    for room_x, room_y in [
        ...
        for i in range(0, room_size):
            for j in range(0, room_size - 1):
                graph.add_edge(
                    (room_x + i, room_y + j, 1), (room_x + i, room_y + j + 1, 1)
                )
    ...
    for room_x, room_y in [
        ...
        for i in range(1, room_size):
            for j in range(0, room_size):
                graph.add_edge(
                    (room_x + i, room_y + j, 2), (room_x + i - 1, room_y + j, 2)
                )
    ...
    graph.add_nodes_from(
        [(door_up_coords[0], door_up_coords[1], dir) for dir in range(4)]
    )
    graph.add_nodes_from(
        [(door_down_coords[0], door_down_coords[1], dir) for dir in range(4)]
    )
    graph.add_nodes_from(
        [(door_left_coords[0], door_left_coords[1], dir) for dir in range(4)]
    )
    graph.add_nodes_from(
        [(door_right_coords[0], door_right_coords[1], dir) for dir in range(4)]
    )

    graph.add_edge(
        (door_up_coords[0], door_up_coords[1], 3),
        (door_up_coords[0], door_up_coords[1], 3),
    )
    graph.add_edge(
        (door_up_coords[0], door_up_coords[1], 0),
        (door_up_coords[0] + 1, door_up_coords[1], 0),
    )
    graph.add_edge(
        (door_up_coords[0] - 1, door_up_coords[1], 0),
        (door_up_coords[0], door_up_coords[1], 0),
    ...
    graph.add_edge(
        (door_up_coords[0], door_up_coords[1], 1),
        (door_up_coords[0], door_up_coords[1], 1),
    )
    graph.add_edge(
        (door_up_coords[0], door_up_coords[1], 2),
        (door_up_coords[0] - 1, door_up_coords[1], 2),
    )
    graph.add_edge(
        (door_up_coords[0] + 1, door_up_coords[1], 2),
        (door_up_coords[0], door_up_coords[1], 2),
    ...
    graph.add_edge(
        (door_down_coords[0], door_down_coords[1], 3),
        (door_down_coords[0], door_down_coords[1], 3),
    )
    graph.add_edge(
        (door_down_coords[0], door_down_coords[1], 0),
        (door_down_coords[0] + 1, door_down_coords[1], 0),
    )
    graph.add_edge(
        (door_down_coords[0] - 1, door_down_coords[1], 0),
        (door_down_coords[0], door_down_coords[1], 0),
    ...
    graph.add_edge(
        (door_down_coords[0], door_down_coords[1], 1),
        (door_down_coords[0], door_down_coords[1], 1),
    )
    graph.add_edge(
        (door_down_coords[0], door_down_coords[1], 2),
        (door_down_coords[0] - 1, door_down_coords[1], 2),
    )
    graph.add_edge(
        (door_down_coords[0] + 1, door_down_coords[1], 2),
        (door_down_coords[0], door_down_coords[1], 2),
    ...
    graph.add_edge(
        (door_left_coords[0], door_left_coords[1], 0),
        (door_left_coords[0], door_left_coords[1], 0),
    )
    graph.add_edge(
        (door_left_coords[0], door_left_coords[1], 3),
        (door_left_coords[0], door_left_coords[1] - 1, 3),
    )
    graph.add_edge(
        (door_left_coords[0], door_left_coords[1] + 1, 3),
        (door_left_coords[0], door_left_coords[1], 3),
    ...
    graph.add_edge(
        (door_left_coords[0], door_left_coords[1], 2),
        (door_left_coords[0], door_left_coords[1], 2),
    )
    graph.add_edge(
        (door_left_coords[0], door_left_coords[1], 1),
        (door_left_coords[0], door_left_coords[1] + 1, 1),
    )
    graph.add_edge(
        (door_left_coords[0], door_left_coords[1] - 1, 1),
        (door_left_coords[0], door_left_coords[1], 1),
    ...
    graph.add_edge(
        (door_right_coords[0], door_right_coords[1], 0),
        (door_right_coords[0], door_right_coords[1], 0),
    )
    graph.add_edge(
        (door_right_coords[0], door_right_coords[1], 3),
        (door_right_coords[0], door_right_coords[1] - 1, 3),
    )
    graph.add_edge(
        (door_right_coords[0], door_right_coords[1] + 1, 3),
        (door_right_coords[0], door_right_coords[1], 3),
    ...
    graph.add_edge(
        (door_right_coords[0], door_right_coords[1], 2),
        (door_right_coords[0], door_right_coords[1], 2),
    )
    graph.add_edge(
        (door_right_coords[0], door_right_coords[1], 1),
        (door_right_coords[0], door_right_coords[1] + 1, 1),
    )
    graph.add_edge(
        (door_right_coords[0], door_right_coords[1] - 1, 1),
        (door_right_coords[0], door_right_coords[1], 1),
...
def find_shortest_path(agent_pos, agent_dir, goal_pos, topology, size):
    ...
    for dir in range(4):
        ...
        paths.append(
            nx.shortest_path(
                graph, source=(*agent_pos, agent_dir), target=(*goal_pos, dir)
            )
        )
...
def find_all_shortest_paths(agent_pos, agent_dir, goal_pos, topology, size):
    ...
    shortest_paths = np.array(paths, dtype=object)[
        np.where(path_lengths == shortest_path_length)[0]
    ]
...
def find_all_action_values(agent_pos, agent_dir, goal_pos, topology, gamma, size):
    ...
    for dir in range(4):
        for p in nx.all_shortest_paths(
            graph, source=(*agent_pos, (agent_dir - 1) % 4), target=(*goal_pos, dir)
        ):
            paths.append(p)
    ...
    if new_pos == goal_pos:
        ...
        for dir in range(4):
            for p in nx.all_shortest_paths(
                graph, source=(*new_pos, agent_dir), target=(*goal_pos, dir)
            ):
                paths.append(p)
