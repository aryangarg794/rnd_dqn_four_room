...
def obs_to_img(obs):
    ...
    lower_right = np.array(
        [np.where(walls.sum(axis=1) == 9)[0][0], np.where(walls.sum(axis=0) == 9)[0][0]]
    )
    shift = np.array([8, 8]) - lower_right
    if (
        np.where(walls.sum(axis=1) == 9)[0][0] == 0
        and np.where(walls.sum(axis=1) == 9)[0][1] == 8
    ):
        shift[0] = 0
    if (
        np.where(walls.sum(axis=0) == 9)[0][0] == 0
        and np.where(walls.sum(axis=0) == 9)[0][1] == 8
    ):
        shift[1] = 0
    ...
    agent_channel = np.repeat(
        np.expand_dims(uncentered_obs[0], axis=2), repeats=3, axis=2
    )
    dir_channel = np.repeat(
        np.expand_dims(uncentered_obs[1], axis=2), repeats=3, axis=2
    )
    goal_channel = np.repeat(
        np.expand_dims(uncentered_obs[3], axis=2), repeats=3, axis=2
    )
    wall_channel = np.repeat(
        np.expand_dims(uncentered_obs[2], axis=2), repeats=3, axis=2
    )
    img = (
        agent_channel * agent_color
        + dir_channel * agent_dir_color
        + goal_channel * goal_color
        + wall_channel * wall_color
    )
...
def obs_to_state(obs):
    ...
    if obs.shape[0] == 3:
        player_loc = (
            np.where(obs[0] == OBJECT_TO_IDX["agent"])[1][0],
            np.where(obs[0] == OBJECT_TO_IDX["agent"])[0][0],
        )
        ...
        doors_pos = (
            *(
                np.where(~(obs[0][:, room_size + 1] == 2))[0]
                - np.array([1, room_size + 2])
            ),
            *(
                np.where(~(obs[0][room_size + 1, :] == 2))[0]
                - np.array([1, room_size + 2])
        ...
        lower_right = np.array(
            [
                np.where(walls.sum(axis=1) == size)[0][0],
                np.where(walls.sum(axis=0) == size)[0][0],
            ]
        )
        shift = np.array([size - 1, size - 1]) - lower_right
        if (
            np.where(walls.sum(axis=1) == size)[0][0] == 0
            and np.where(walls.sum(axis=1) == size)[0][1] == size - 1
        ):
            shift[0] = 0
        if (
            np.where(walls.sum(axis=0) == size)[0][0] == 0
            and np.where(walls.sum(axis=0) == size)[0][1] == size - 1
        ):
            shift[1] = 0

        uncentered_obs = np.roll(obs, tuple(shift), axis=(1, 2))
        player_loc = (
            np.where(uncentered_obs[0] == 1)[1][0],
            np.where(uncentered_obs[0] == 1)[0][0],
        )
        player_dir_loc = (
            np.where(uncentered_obs[1] == 1)[1][0],
            np.where(uncentered_obs[1] == 1)[0][0],
        )
        ...
        doors_pos = (
            *(np.where(walls[:, room_size + 1] == 0)[0] - np.array([1, room_size + 2])),
            *(np.where(walls[room_size + 1, :] == 0)[0] - np.array([1, room_size + 2])),
        )
