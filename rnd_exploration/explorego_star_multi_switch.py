        ...
        while len(explorego) <= dataset_size:
            ...
            while not done:
                ...
                q = find_all_action_values(
                    state[:2], state[2], target_pos, state[5:], 0.99, size
                )
                q = np.array(q)
                action = q.argmax()
                ...
                if len(explorego) < warmupsteps and warmup:
                    ...
                    if render:
                        ...
                        past_pos.append(agent_pos)
                    rnd_net.observe(obs)
                elif rnd_val - rms.avg >= alpha * rms.std or np.array_equal(
                    agent_pos, aux_pos
                ):  # swap to record mode
                    record = True
                    target_pos = goal_pos
            ...
            # print(f'Current size of dataset: {len(explorego):08d} | Current Context {current_context:03d} | Current Uniqueness {explorego.ratio_unique_trans:.4f} | Val {rnd_val:.4f} | Current Target: {target} | Recording: {record}', end='\r')
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--size", type=int, default=1500, help="size of dataset")
