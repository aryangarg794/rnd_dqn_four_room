    ...
    def __init__(
        ...
        **kwargs
        ...
        if use_action:
            self.net.append(
                nn.Linear(
                    feature_units + np.prod(action_space.shape, dtype=np.int64),
                    hidden_layers[0],
                )
            )
        else:
        ...
        for next_dim, prev_dim in zip(hidden_layers[1:], hidden_layers[:-1]):
            self.net.extend([nn.Linear(prev_dim, next_dim), nn.LeakyReLU()])
...
class RNDNetwork:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.rnd_net = BaseNetwork(
            use_actions, env.observation_space, env.action_space, use_cnn=use_cnn
        ).to(device)
    ...
    def get_error(self, state: Tensor, action: Tensor = None) -> float:
        ...
        return (
            self.loss(self.rnd_net(states, action), self.target_net(states, action))
            .sum(dim=-1)
            .detach()
        )
