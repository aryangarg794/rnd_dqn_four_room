...
def train_explorego(
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1))):
        ...
        pbar.set_description(
            f"Training RND DQN | Uniqueness: {buffer.ratio_unique_trans:.4f} | Regression Exp: {(scores[-1] if len(scores) > 0 else 0):.4f} | Items added: {items_added} | Context: {current_context}"
        )
