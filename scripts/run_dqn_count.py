...
def train_dqn_count(
    ...
    counter_moving = MovingCountBasedUncertainty(
        capacity=args.capacity, return_ones=return_ones, device=args.device
    )
    ...
    contexts = []
    last_ep = LastEpisode(
        args.env.observation_space.shape, capacity=last_episode_len, device=args.device
    )
    last_expl_ep = LastEpisode(
        args.env.observation_space.shape, capacity=last_episode_len, device=args.device
    )
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1), disable=debug)):
        ...
        if (
            (dqn_val - rms_dqn.avg >= alpha * rms_dqn.std or np.random.random() < eps)
            and not record
        ...
        for _ in range(gradient_steps):
            ...
            batch_actions = torch.cat(
                [batch_actions, last_action, last_action_expl], dim=0
            )
            batch_primes = torch.cat(
                [batch_primes, last_obs_primes, last_obs_primes_expl], dim=0
            )
            batch_next_actions = torch.cat(
                [batch_next_actions, last_next_actions, last_next_actions_expl], dim=0
            )
            batch_dones = torch.cat([batch_dones, last_dones, last_dones_expl], dim=0)
            batch_rewards = torch.cat(
                [batch_rewards, last_rewards, last_rewards_expl], dim=0
            )
        ...
        pbar.set_description(
            f"Training RND Count | Uniqueness: {agent.buffer.ratio_unique_trans:.4f} | Regression Exp: {(scores[-1] if len(scores) > 0 else 0):.4f} | Items added: {items_added} | Context: {current_context}"
        )
