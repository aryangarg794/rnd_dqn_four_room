...
def train_dqn_count(
    ...
    rnd_net = RNDNetwork(env=args.env, lr=args.lr_rnd, device=args.device)

    counter_moving = MovingCountBasedUncertainty(
        capacity=args.capacity, return_ones=True
    )
    ...
    rnd_step = 0
    rnd_seen_obs = torch.randn((rnd_steps, *args.env.observation_space.shape)).to(
        device=args.device
    )
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1), disable=debug)):
        ...
        if np.array_equal(target_pos, aux_pos) and not np.array_equal(
            agent_pos, aux_pos
        ):
        ...
        if step < warmupsteps or record:
            ...
            dqn_val - rms_dqn.avg >= alpha * rms_dqn.std and not record
        ):  # swap to record mode
            ...
            np.array_equal(agent_pos_after, aux_pos)
            or np.array_equal(agent_pos, aux_pos)
        ) and not record:
            target_pos = goal_pos

        if render and step >= num_timesteps - 1000:
            env.get_wrapper_attr("set_aux")(
                aux_pos
            )  # cannot add beforehand or else included in obs
            agent_col = (
        ...
        if step % update_freq == 0:
            (
                ...
                batch_next_actions,
                batch_dones,
            ) = agent.buffer.sample(batch_size=batch_size)

            batch_obs = torch.cat([batch_obs, obs_torch], dim=0)
            action_torch = torch.tensor(
                action, dtype=torch.int64, device=args.device
            ).view(1, -1)
            batch_actions = torch.cat([batch_actions, action_torch], dim=0)
            obs_prime_torch = (
                torch.from_numpy(obs_prime).to(device=args.device).unsqueeze(dim=0)
            )
            batch_primes = torch.cat([batch_primes, obs_prime_torch], dim=0)
            next_action_torch = torch.tensor(
                next_action, dtype=torch.int64, device=args.device
            ).view(1, -1)
            batch_next_actions = torch.cat(
                [batch_next_actions, next_action_torch], dim=0
            )
            done_torch = torch.tensor(done, dtype=torch.int, device=args.device).view(
                1, -1
            )
            ...
            with torch.no_grad():
                ...
                target_vals = agent.target_net(batch_primes).gather(
            ...
            grad_norm = torch.nn.utils.clip_grad_norm_(
                agent.net.parameters(), grad_norm
            )
        ...
        if step % rnd_steps == 0:
            ...
            rnd_seen_obs = torch.zeros(
                (rnd_steps, *args.env.observation_space.shape)
        ...
        if done:
            ...
            paths = find_all_shortest_paths(
                state[:2], state[2], aux_pos, state[5:], size
            )
            path_index = np.random.randint(low=0, high=len(paths))
            actions = compute_actions(paths[path_index])
        ...
        pbar.set_description(
            f"Training RND Count | Uniqueness: {agent.buffer.ratio_unique_trans:.4f} | Items added: {items_added} | Context: {current_context} | Last RND {value:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--timesteps", type=int, default=int(1e5), help="timesteps"
    )
    parser.add_argument(
        "-f", "--dir", type=str, default="dqn_count_rnd_test", help="save name"
    )
    ...
    parser.add_argument(
        "-ag", "--lr_agent", type=float, default=1e-3, help="lr for dqn agent"
    )
    ...
    parser.add_argument(
        "-s", "--replaysize", type=int, default=int(5e4), help="size of replay buffer"
    )
