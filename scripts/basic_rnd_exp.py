...
def train_basic_rnd(
    ...
    rnd_net = RNDNetwork(env=args.env, lr=args.lr, device=args.device)
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1))):
        ...
        if step < warmupsteps or record:
            ...
            rnd_val - rms_val.avg >= alpha * rms_val.std and not record
        ):  # swap to record mode
            switches += 1
        ...
        pbar.set_description(
            f"Training RND DQN | Uniqueness: {buffer.ratio_unique_trans:.4f} | Last Regression Exp: {(scores[-1] if len(scores) > 0 else 0):.4f} | Total Items added: {items_added} | Current Context: {current_context} | RND Val: {rnd_val:.4f} | Avg: {rms_val.avg:.4f} | STD: {rms_val.std:.4f} | Switches: {switches} | Value: {value:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--window", type=int, default=2500, help="window size of rms_val"
    )
