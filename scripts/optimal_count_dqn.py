...
def train_opt_count(
    ...
    counter_moving = MovingCountBasedUncertainty(
        capacity=args.capacity, return_ones=return_ones
    )
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1), disable=debug)):
        ...
        with torch.no_grad():
            ...
            dqn_val = compute_q_value(
                obs, goal_action, current_context, counter_moving, gamma
            )
        ...
        ) and not record:  # swap to record mode
            switches += 1
