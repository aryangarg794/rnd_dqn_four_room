...
def train_basic_count(
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1))):
        ...
        pbar.set_description(
            f"Training Count | Uniqueness: {buffer.ratio_unique_trans:.4f} | Regression Exp: {(scores[-1] if len(scores) > 0 else 0):.4f} | Items added: {items_added} | Context: {current_context} | RND Val: {uncertainty:.4f} | Avg: {rms_val.avg:.4f} | STD: {rms_val.std:.4f} "
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-f", "--dir", type=str, default="basic_count", help="save name"
    )
