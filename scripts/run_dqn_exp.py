...
def train_dqn_rnd(
    ...
    trajs_added = 0
    rnd_step = 0
    rnd_seen_obs = torch.zeros((rnd_steps, *args.env.observation_space.shape)).to(
        device=args.device
    )
    ...
    for step in (pbar := tqdm(range(1, num_timesteps + 1), disable=debug)):
        ...
        if step < warmupsteps or record:
            ...
            if debug:
                ...
                traj_in.append(agent.buffer.unique_trans.has(Transition(obs, q)))
        ...
        pbar.set_description(
            f"Training RND DQN | Uniqueness: {agent.buffer.ratio_unique_trans:.4f} | Last Regression Exp: {(scores[-1] if len(scores) > 0 else 0):.4f} | Total Items added: {items_added} | Current Context: {current_context} | RND Val: {dqn_val:.4f} | Avg: {rms_dqn.avg:.4f} | STD: {rms_dqn.std:.4f} | Switches: {switches} | Value: {value:.4f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-t", "--timesteps", type=int, default=int(1e6), help="timesteps"
    )
