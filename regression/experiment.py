...
class RegressionModel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i, layer in enumerate(hidden_layers[:-1]):
            self.layers.extend([nn.Linear(layer, hidden_layers[i + 1]), activation()])
    ...
    def run(
        ...
        X_train = torch.as_tensor(
            np.array(dataset.X), dtype=torch.float32, device=self.device
        )
        y_train = torch.as_tensor(
            np.array(dataset.Y), dtype=torch.float32, device=self.device
        )
        ...
        for step in range(timesteps + 1):
            ...
            if step % print_freq == 0:
                ...
                print(
                    f"For dataset: {label} || Step: {step} / {timesteps} | Loss: {loss.item():.3f} | Val Rewards: {val_reward:.3f} | Hours left: {time_left:.1f}",
                    end="\r",
                )
...
class Experiment:
            ...
            for seed in self.seeds:
                ...
                for i, dataset in enumerate(datasets):
                    ...
                    val_rewards = model.run(
                        ...
                        print_freq=self.val_freq,
                        label=labels[i],
                        batch_size=self.batch_size,
                    )
                    results[i].append(val_rewards)
                    test_results[i].append(
                        model.validation(self.test_env, val_steps=200)
                    )
    ...
    def plot_result(self, results: list, labels: list):
        def plot_with_ci(ax, data, label, q=5):
            ...
            ax.fill_between(
                np.arange(0, self.timesteps + self.val_freq, self.val_freq),
                ci_lower,
                ci_upper,
                alpha=0.2,
            )
