...
from utils.record_scores import (
    record_dqn_scores,
    get_rnd_scores,
    get_q_optimal,
    record_uncertainty_scores,
    record_uvu_scores,
)
...
def simulate_trajectory(
    ...
    uncertainty_map, _, uncertainty_map_dirs = record_uncertainty_scores(
        counter, random_context, 0
    )
    ...
    relevant_buffer_states = find_states(
        counter.all_states, timesteps, counter.capacity
    )
    ...
    if rnd:
        ...
        rnd_scores, _ = get_rnd_scores(
            rnd_net, random_context, env_range=0, device=device
        )
    ...
    while not done:
        ...
        render = env.unwrapped.render(
            highlight_mask=ep_highlight_mask, colors=ep_colors, agent_col=agent_col
        )
        ...
        plot_env_heatmap(
            (
                ...
                if normalized
                else dqn_scores_dirs[0].squeeze()
            ),
            context_info,
            "DQN Scores",
        ...
        plot_env_heatmap(
            (
                ...
                if normalized
                else dqn_scores_dirs[1].squeeze()
            ),
            context_info,
            "DQN Scores",
        ...
        plot_env_heatmap(
            (
                ...
                if normalized
                else dqn_scores_dirs[2].squeeze()
            ),
            context_info,
            "DQN Scores",
        ...
        plot_env_heatmap(
            (
                ...
                if normalized
                else dqn_scores_dirs[3].squeeze()
            ),
            context_info,
            "DQN Scores",
        ...
        print(
            f'Step {current_step}: {"Exploring..." if not record else "Recording..."}',
            end="\r",
        )
        current_step += 1
