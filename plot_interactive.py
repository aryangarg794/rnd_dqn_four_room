...
def plot_env_heatmap(
    ...
    for r in range(matrix_data.shape[0]):
        for c in range(matrix_data.shape[1]):
            ...
            matrix_data[r, c] = (
                str(int(matrix[r, c])) if intize else str(round(matrix[r, c], 2))
            )
...
def plot_timestep_matplotlib(
    file_name: str,
    timestep: int,
    timestep_idx: int,
    annot_expl: bool = True,
    results=None,
    env=None,
):
    """
    ...
    relevant_buffer_states = find_states(
        buffer_moving.all_states, timestep, buffer_moving.capacity
    )
    ...
    # moving switch count
    plot_env_heatmap(
        buffer_switch_counts[context],
        context_info,
        "Switch Counts",
        f"Switches Heatmap for context {context}",
        axes[1, 0],
    )
    # moving buffer count
    plot_env_heatmap(
    ...
    # full switches
    plot_env_heatmap(
        full_switches[context],
        context_info,
        "Switch Counts (full)",
        f"Switches Heatmap (full) for context {context}",
        axes[2, 0],
    )
    # full buffer
    plot_env_heatmap(
        buffer_full.counts_matrix[context],
        context_info,
        "Buffer Counts (full)",
        f"Buffer Heatmap (full) for context {context}",
        axes[2, 1],
    )
    # explored states
    plot_env_heatmap(
        explored_states[context],
        context_info,
        "Explore Counts",
        f"Explored States (full) for context {context}",
        axes[3, 0],
        annot=annot_expl,
    )
    # aux selected states
    plot_env_heatmap(
        ...
        context_info,
        "Aux Counts",
        f"Aux Selected Heatmap (full) for context {context}",
        axes[3, 1],
        annot=annot_expl,
    )
...
def create_dash_app(
    file_name: str, timesteps: list, annot_expl: bool = True, port: int = 8050
):
    ...
    app.layout = html.Div(
        [
            html.Div(
                [
                    html.H1(
                        "Count Based Timestep Visualization",
                        style={
                            "textAlign": "center",
                            "color": "#fff",
                            "marginBottom": "10px",
                    ...
                    html.P(
                        "Interactive exploration of agent behavior across timesteps",
                        style={
                            "textAlign": "center",
                            "color": "#fff",
                            "fontSize": "1.1em",
...
if __name__ == "__main__":
    ...
    create_dash_app(
        "dqn_count_test_seed_0_100000", timesteps_to_plot, annot_expl=True, port=8050
    )
